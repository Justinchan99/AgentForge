...
安装器使用 winget 安装系统工具，并使用 npm 安装 OpenCode/pyright。仅当目标工程包含 C/C++ 时安装 clangd，仅当目标工程包含 Python 时安装 pyright。

在安装前直接从 AgentForge 源码目录运行时，请在 PowerShell 中使用 `./agentforge.cmd <command>`。无扩展名的 `./agentforge` 是供 Linux 和 macOS Shell 使用的 POSIX 启动器。
